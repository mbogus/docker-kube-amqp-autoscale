...
if [ -n "$AUTOSCALE_INCREASE_LIMIT" ]; then
    PRG_ARGS="${PRG_ARGS} --increase-limit=${AUTOSCALE_INCREASE_LIMIT}"
fi

if [ -n "$AUTOSCALE_DECREASE_LIMIT" ]; then
    PRG_ARGS="${PRG_ARGS} --decrease-limit=${AUTOSCALE_DECREASE_LIMIT}"
fi
